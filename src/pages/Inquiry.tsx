  ...
  const onSubmit = async (data: InquiryForm) => {
    ...
    try {
      ...
      if (error.text?.includes("recipients address is empty")) {
        toast.error("EmailJS template not configured properly. Please set recipient email in template.");
      } else {
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            {(!import.meta.env.VITE_EMAILJS_SERVICE_ID || import.meta.env.VITE_EMAILJS_SERVICE_ID === 'service_abc123') && (
              <div className="mt-2 p-2 bg-yellow-100 border border-yellow-400 rounded text-sm text-yellow-800">
                ⚠️ EmailJS not configured. Please set up your EmailJS credentials.
              </div>
            )}
          </CardDescription>
        </CardHeader>
